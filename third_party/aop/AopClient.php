...
class AopClient {
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		...
		$logger = new LtLogger;
		$logger->conf["log_file"] = rtrim(APPPATH, '\\/') . '/' . "logs/aop_comm_err_" . $this->appId . "_" . date("Y-m-d") . ".log";
	...
	public function execute($request, $authToken = null) {
		...
		if (isset ($respObject->code)) {
			$logger = new LtLogger;
			$logger->conf["log_file"] = rtrim(APPPATH, '\\/') . '/' . "logs/aop_biz_err_" . $this->appId . "_" . date("Y-m-d") . ".log";
			$logger->log(array (
				date("Y-m-d H:i:s"),
				$resp
			));
